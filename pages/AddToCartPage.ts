...
export class AddToCartPage {
    ...
    constructor(page:Page){
        ...
        this.purchaseSection = page.getByRole("region", {name: "Buy Palworld", exact: true});

        // Add to cart button
        this.addToCartBtn = this.purchaseSection.getByRole("button", {name: "Add to Cart"});
    ...
    async getItemName(): Promise<string> {
        try {
            ...
            const title = (await this.itemName.innerText()).replace("Buy ", "").trim();
    ...
    async getConfirmation(): Promise< {successfulTip: string; finalName: string; finalPrice: string} > {
        try{
            ...
            console.log(`Add game to cart successfully, Name: ${finalName}, Price: ${finalPrice}`);
            return result;
