...
test.describe("Steam Footer Links - User Flow", () => {
  ...
  for (const item of footerLinkTests) {
    test(`user clicks ${item.name} from homepage, verifies header & body, and returns home`, async ({
      context,
      page,
    }) => {
  ...
  test("user navigates to Privacy Policy and switches language to Spanish", async ({
    context,
    page,
  }) => {
    const pomLegal = new POM_Legal(page);
    ...
    await pomLegal.navigateToHome();
    const targetPage = await pomLegal.clickFooterLink(
      'a[href*="privacy_agreement"]:visible, a:has-text("Privacy"):visible',
      context,
    );

    // Handle language selection specifically on the Privacy Policy page
    const spanishLink = targetPage
      .locator(
        '#languages a[href*="spanish"], .privacy_policy_languages a[href*="spanish"], a[href*="privacy_agreement/spanish"]',
      )
      .first();
    ...
    if (await spanishLink.isVisible()) {
      ...
      await targetPage.goto(pomLegal.urls.PRIVACY_SPANISH, {
    ...
    // Verify Spanish header using safe heading definitions
    const spanishHeader = targetPage
      .locator("h1, h2, h3, [role='heading'], .privacy_header_title")
  ...
  test("user navigates to Accessibility page and downloads compliance report", async ({
    page,
  }) => {
    const pomLegal = new POM_Legal(page);
