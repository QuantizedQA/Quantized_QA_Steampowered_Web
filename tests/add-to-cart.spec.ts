...
test.describe("Add to Cart", () => {
  for (const game of [games.palworld, games.valheim]) {
    test(`Add ${game.name} to the cart successfully`, async ({ page }) => {
      ...
      await expect(addToCartPage.successfulTip).toHaveText(
        "Added to your cart!",
      );
